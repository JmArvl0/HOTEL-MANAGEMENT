...
const createSchema=z.object({number:z.string().trim().min(1).max(20),type:z.string().trim().min(1).max(100),floor:z.coerce.number().int().min(0).max(200),wing:z.string().trim().max(80).optional(),designation:z.string().trim().max(120).optional(),active:z.boolean().optional().default(true),reason:z.string().trim().max(500).optional()});
export async function POST(request:Request){
  const c=await guardCatalog();if(adminGuardFailed(c))return c;
  const p=createSchema.safeParse(await request.json());if(!p.success)return NextResponse.json({error:"Enter a valid room number, floor, and room type."},{status:400});
  const v=p.data,{data,error}=await c.client.rpc("admin_create_room",{p_number:v.number,p_type:v.type,p_floor:v.floor,p_wing:v.wing??null,p_designation:v.designation??null,p_active:v.active,p_reason:v.reason?.trim()?v.reason:null,p_actor_user_id:c.actorId});
