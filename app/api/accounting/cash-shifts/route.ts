...
export async function GET(){const context=await guardFinancial(canOperateCashShift,"Cash handling access required.");if(guardFailed(context))return context;
 const{data:shift}=await context.client.from("cash_shifts").select("id,location,opening_amount,opened_at").eq("staff_user_id",context.actorId).eq("status","open").maybeSingle();
 return NextResponse.json({data:{open:Boolean(shift),shift:shift??null}})}
