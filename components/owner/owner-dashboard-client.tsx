...
import { Activity, BarChart3, BedDouble, Building2, CarTaxiFront, ChevronDown, ChevronRight, CircleDollarSign, ClipboardCheck, FileText, Image, KeyRound, LogOut, PanelLeftClose, QrCode, Settings, ShieldCheck, Sparkles, Users } from "lucide-react";
...
function Admins({ rows, currentId, createAdmin, action }: { rows: Row[]; currentId: string; createAdmin: () => void; action: (item: Row, action: "status" | "recovery") => void }) {
  const ordered=sortTableRows(rows,(item)=>String(item.name??item.email??""));
  const page=useTablePagination(ordered);
  return <><Title eyebrow="Protected administration" title="Owner and Admin accounts" subtitle="Owner governs Admin accounts. Owner self-lockout and last-active-Owner protections are enforced server-side." action={<button className="btn btn-accent" onClick={createAdmin}>Create Admin</button>}/><div className="data-panel"><div className="table-scroll"><table aria-label="Staff accounts"><thead><tr>{["Name", "Email", "Role", "Status", "Recovery", "Version", "Actions"].map((value) => <th key={value}>{value}</th>)}</tr></thead><tbody>{page.rows.map((item) => <tr key={String(item.id)}><td><strong>{label(item.name)}</strong></td><td>{label(item.email)}</td><td>{roleLabel(item.role)}</td><td><span className={`badge ${item.account_status}`}>{label(item.account_status)}</span></td><td>{item.recovery_required ? "Required" : "No"}</td><td>{label(item.auth_version)}</td><td>{item.role === "admin" && item.id !== currentId ? <div className="reservation-actions"><button className="table-action" onClick={() => action(item, "status")}>Lifecycle</button><button className="table-action" onClick={() => action(item, "recovery")}>Recovery</button></div> : <small>Protected Owner</small>}</td></tr>)}</tbody></table></div><TablePagination {...page} onPageChange={page.setPage} noun="accounts" note="Alphabetical by account name."/></div></>;
}
...
function Exceptions({ rows, review }: { rows: Row[]; review: (item: Row, decision: "approve" | "reject") => void }) {
  ...
  return <><Title eyebrow="Authorization, not execution" title="Owner-level exceptions" subtitle="High-risk Manager escalations. Approval authorizes the responsible department; it does not execute their work."/><div className="data-panel"><div className="table-scroll"><table aria-label="Owner exception requests"><thead><tr>{["Requested", "Type", "Reservation", "Department", "Severity", "Reason", "Escalation", "Status", "Actions"].map((value) => <th key={value}>{value}</th>)}</tr></thead><tbody>{page.rows.map((item) => <tr key={String(item.id)}><td>{label(item.requested_at)}</td><td><strong>{label(item.request_type)}</strong></td><td>{label(item.reservation_reference)}<br/><small>{label(item.guest_name)}</small></td><td>{label(item.department)}</td><td><span className={`badge ${item.severity}`}>{label(item.severity)}</span></td><td>{label(item.reason)}</td><td>{label(item.owner_escalation_reason)}<br/><small>{label(item.escalator_name)}</small></td><td><span className={`badge ${item.status}`}>{label(item.status)}</span><br/><small>{label(item.execution_status)}</small></td><td>{item.status === "pending" ? <div className="reservation-actions"><button className="table-action" onClick={() => review(item, "approve")}>Authorize</button><button className="table-action danger-action" onClick={() => review(item, "reject")}>Reject</button></div> : <small>{label(item.decision_reason)}</small>}</td></tr>)}</tbody></table></div>{rows.length === 0 && <div className="empty"><ClipboardCheck/><h3>No Owner-level exceptions</h3><p>Managers have not escalated a high-risk decision.</p></div>}<TablePagination {...page} onPageChange={page.setPage} noun="exceptions" note="Executive escalation order retained."/></div></>;
}
...
function Table({ title, rows, columns }: { title: string; rows: Row[]; columns: string[] }) {
  const page=useTablePagination(rows);
  return <article className="data-panel owner-table"><div className="panel-heading"><div><h3>{title}</h3><p>{rows.length} authoritative record{rows.length === 1 ? "" : "s"}</p></div></div><div className="table-scroll"><table aria-label="Owner records"><thead><tr>{columns.map((column) => <th key={column}>{label(column)}</th>)}</tr></thead><tbody>{page.rows.map((row, index) => <tr key={String(row.id ?? `${title}-${index}`)}>{columns.map((column) => <td key={column}>{formatValue(column, row[column])}</td>)}</tr>)}</tbody></table></div>{rows.length === 0 && <div className="empty"><p>No matching records.</p></div>}<TablePagination {...page} onPageChange={page.setPage} noun="records" note="Executive source order retained."/></article>;
}
