...
export function BookingSummary({ roomType, checkIn, checkOut, guests, nights, rate, total, lines }: { roomType:string;checkIn:string;checkOut:string;guests:number;nights:number;rate:number;total:number;lines?:{label:string;amount:number}[] }) { const peso=(v:number)=>new Intl.NumberFormat("en-PH",{style:"currency",currency:"PHP",maximumFractionDigits:0}).format(v);return <aside className="booking-summary"><p className="eyebrow">Your stay</p><h2>{roomType}</h2><dl><div><dt>Check-in</dt><dd>{checkIn}</dd></div><div><dt>Check-out</dt><dd>{checkOut}</dd></div><div><dt>Guests</dt><dd>{guests}</dd></div><div><dt>Nights</dt><dd>{nights}</dd></div><div><dt>Nightly rate</dt><dd>{peso(rate)}</dd></div>{(lines??[]).map((line)=><div key={line.label}><dt>{line.label}</dt><dd>{peso(line.amount)}</dd></div>)}<div className="summary-total"><dt>Stay total</dt><dd>{peso(total)}</dd></div></dl><small>Taxes and service charges are currently included at ₱0 under the configured hotel policy.</small></aside>; }