...
export function BookingPageFrame({ session, step, breadcrumb, children }: { session: Session | null; step?: string; breadcrumb?: BreadcrumbItem[]; children: React.ReactNode }) {
  ...
  if (session?.user.role === "guest") return <CustomerShell user={session.user} sessionExpiresAt={session.sessionExpiresAt}><div className="customer-booking-flow">{breadcrumb && <div className="booking-flow-navigation"><Breadcrumb items={breadcrumb}/><BackButton to={stepLink?.href} label={backLabel} className="booking-back--flow"/></div>}{children}</div></CustomerShell>;
