    ...
    <div className="booking-form-options">
      ...
      <div className="request-options-list">
        ...
        {needRide && (<div className="booking-form-grid transport-ride-fields">
          ...
          {(serviceType === "PICKUP" || serviceType === "ROUND_TRIP") && <label className="wide">{serviceType === "ROUND_TRIP" ? "Arrival pickup location" : "Pickup location"}<input name="pickupLocation" required minLength={5} maxLength={200} placeholder="e.g. NAIA Terminal 3, Pasay City" defaultValue={initialTransport?.pickupLocation ?? ""}/></label>}
          {serviceType === "DROPOFF" && <label className="wide">Destination<input name="dropoffLocation" required minLength={5} maxLength={200} placeholder="e.g. NAIA Terminal 3, Pasay City" defaultValue={initialTransport?.dropoffLocation ?? ""}/></label>}
          {serviceType === "ROUND_TRIP" && (<>
            <label className="wide">Departure destination<input name="returnLocation" required minLength={5} maxLength={200} placeholder="e.g. NAIA Terminal 1, Pasay City" defaultValue={initialTransport?.returnLocation ?? ""}/></label>
            <label>Return date<input name="returnDate" type="date" required min={checkIn} max={checkOut} defaultValue={initialTransport?.returnDate ?? ""}/></label>
            <label>Return time<input name="returnTime" type="time" required defaultValue={initialTransport?.returnTime ?? ""}/></label>
          </>)}
          <label>Pickup date<input name="pickupDate" type="date" required min={checkIn} max={checkOut} defaultValue={initialTransport?.pickupDate ?? ""}/></label>
          <label>Pickup time<input name="pickupTime" type="time" required defaultValue={initialTransport?.pickupTime ?? ""}/></label>
          <label>Passengers<input name="passengerCount" type="number" min={1} max={20} defaultValue={initialTransport?.passengerCount ?? (guests > 20 ? 20 : guests)} required/></label>
          <label className="wide">Pickup time or instructions (optional)<input name="rideNote" maxLength={500} placeholder="e.g. flight lands 6:00 PM, 2 large suitcases" defaultValue={initialTransport?.specialInstructions ?? ""}/></label>
        </div>)}
      </div>
    </div>
