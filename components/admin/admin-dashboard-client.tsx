...
export function UsersView({rows,create,action}:{rows:RecordItem[];create:()=>void;action:(x:RecordItem,a:"status"|"role"|"metadata"|"recovery")=>void}){const[role,setRole]=useState("all");const[status,setStatus]=useState("all");const[recovery,setRecovery]=useState("all");const[department,setDepartment]=useState("all");const[search,setSearch]=useState("");
 ...
 <div className="reservation-filters approval-toolbar"><div className="approval-filters"><label className="approval-search"><Search size={15}/><input value={search} onChange={event=>setSearch(event.target.value)} aria-label="Search accounts" placeholder="Search name, email, department, or reference..."/></label><label>Role<select value={role} onChange={event=>setRole(event.target.value)}><option value="all">All roles</option>{roles.map(value=><option key={value} value={value}>{roleLabel(value)}</option>)}</select></label><label>Status<select value={status} onChange={event=>setStatus(event.target.value)}><option value="all">All statuses</option>{STATUS_OPTIONS.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>Recovery<select value={recovery} onChange={event=>setRecovery(event.target.value)}><option value="all">All accounts</option><option value="required">Recovery required</option><option value="no">No recovery needed</option></select></label><label>Department<select value={department} onChange={event=>setDepartment(event.target.value)}><option value="all">All departments</option>{departments.map(value=><option key={value} value={value}>{label(value)}</option>)}</select></label></div></div>
