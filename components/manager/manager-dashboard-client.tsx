...
import { Activity, AlertTriangle, BedDouble, Bell, Boxes, Building2, CalendarDays, CarTaxiFront, CheckCircle2, ChevronDown, ChevronRight, CircleDollarSign, ClipboardCheck, DoorClosed, Download, Eye, FileText, Gauge, Landmark, ListChecks, LogIn, LogOut, Plus, QrCode, ReceiptText, Scale, Search, Settings, Sparkles, TrendingUp, Users, Wallet, Wrench } from "lucide-react";
...
export function ManagerSidebarNav({ items, section, onSelect, openTasks }: { items: typeof nav; section: Section; onSelect: (section: Section) => void; openTasks: number }) {
  ...
  return <nav aria-label="Modules">{groupedNav(items).map((group)=>{const open=isGroupOpen(openGroups,group,section);const groupBadge=group.items.reduce((sum,item)=>sum+(item.section==="housekeeping_tasks"?openTasks:0),0);return <div className="nav-group-wrap" key={group.id}>
        <button className="nav-caption nav-group-header" aria-expanded={open} aria-controls={`nav-group-${group.id}`} onClick={()=>setOpenGroups((prev)=>{const next={...prev,[group.id]:!(prev[group.id]??false)};window.localStorage.setItem("haven-sidebar-groups",JSON.stringify(next));return next})}><span className="nav-group-label">{group.label}</span>{!open&&groupBadge>0&&<i>{groupBadge}</i>}<ChevronRight size={13} className="nav-group-chevron" aria-hidden="true"/></button>
        <div className={`nav-group${open?" open":""}`} id={`nav-group-${group.id}`}><div className="nav-group-items">{group.items.map(({label: text,section: target,icon:Icon})=><button key={target} className={section===target?"active":""} onClick={()=>onSelect(target)} title={text}><Icon size={18}/><span className="nav-label">{text}</span>{target==="housekeeping_tasks"&&openTasks>0&&<i>{openTasks}</i>}</button>)}</div></div>
      </div>})}</nav>;
}
...
export default function ManagerDashboardClient({ user }: { user: User }) {
  ...
  return <div className={`app-shell${collapsed ? " sidebar-collapsed" : ""}`}>
    ...
    <aside className={`sidebar${menu ? " open" : ""}${collapsed ? " collapsed" : ""}`}>
      ...
      <ManagerSidebarNav items={visibleNav} section={section} onSelect={(target)=>{setSection(target);setMenu(false)}} openTasks={Number(dashboard?.metrics.openTasks||0)}/>
...
function ReservationDetailModal({detail,close,checkIn,closeReservation,collectPayment,postCharge,assignRoom,changeRoom,extendStay,updateGuest,routeRequest,checkOut,reverseCharge,recordAdjustment,generateDocument,canFinancial,canManage,canAdjust,canIssueDocument,oversight,canViewTransportation,onReviewException}:{detail:ReservationDetail;close:()=>void;checkIn:(item:RecordItem)=>void;closeReservation:(status:"cancelled"|"no_show")=>void;collectPayment:(item:RecordItem)=>void;postCharge:(item:RecordItem)=>void;assignRoom:(item:RecordItem)=>void;changeRoom:(item:RecordItem)=>void;extendStay:(item:RecordItem)=>void;updateGuest:(item:RecordItem)=>void;routeRequest:(item:RecordItem)=>void;checkOut:(item:RecordItem)=>void;reverseCharge:(item:RecordItem)=>void;recordAdjustment:(reservationId:string)=>void;generateDocument:(payload:{documentType:"receipt";paymentId:string}|{documentType:"folio";reservationId:string})=>void;canFinancial:boolean;canManage:boolean;canAdjust:boolean;canIssueDocument:boolean;oversight?:boolean;canViewTransportation?:boolean;onReviewException?:(item:RecordItem)=>void}){
