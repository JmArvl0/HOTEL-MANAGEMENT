...
import { Activity, AlertTriangle, BedDouble, Bell, Boxes, Building2, CalendarCheck, CalendarDays, CarTaxiFront, CheckCircle2, ChevronDown, ChevronRight, CircleDollarSign, ClipboardCheck, Clock, DoorClosed, Eye, FileText, Gauge, Landmark, ListChecks, LogIn, LogOut, PanelLeftClose, Plus, QrCode, ReceiptText, Scale, Search, Settings, Sparkles, TrendingUp, Users, Wallet, Wrench } from "lucide-react";
...
import Reports from "@/components/manager/performance-reports";
...
// Header bell count — the single authoritative aggregate over the live alert
...
export function countUniqueNotifications(notifications: readonly { id: unknown }[] | null | undefined) {
  const seen = new Set<string>();
  for (const notice of notifications ?? []) seen.add(String(notice?.id));
  return seen.size;
}
// Header notification bell — exactly ONE badge for the aggregate live-alert
...
export function HeaderNotificationBell({ count, expanded, onToggle }: { count: number; expanded: boolean; onToggle: () => void }) {
  return (
    <button className="icon-button" aria-label={count ? `Notifications, ${count} current` : "Notifications"} aria-expanded={expanded} onClick={onToggle}>
      <Bell size={19} />
      {count > 0 && <i className="nav-badge" aria-hidden="true">{badgeLabel(count)}</i>}
    </button>
  );
}
...
export default function ManagerDashboardClient({ user }: { user: User }) {
  ...
  const load = useCallback(async (quiet=false) => {
    ...
    if(section==="room_types"||section==="transport_services"||section==="request_types"||section==="transportation"||section==="insights"||section==="ai"||section==="staff_duty"||(section==="guest_requests"&&["front_desk","manager","housekeeping"].includes(user.role))){if(!quiet)setLoading(false);return}
    const accounting=isAccountingSection(section),approvals=section==="approvals";
    const url=section==="overview"||section==="reports"?"/api/manager_dashboard":accounting?"/api/accounting/ledger":approvals?"/api/manager/approvals":`/api/resources/${section}`;
    try{
      const res=await fetch(url,{cache:"no-store"});
      const body=await res.json().catch(()=>({}));
      if(!res.ok)throw new Error(body.error??"Unable to load this module.");
      if(accounting)setLedger(body.data);else if(section==="overview"||section==="reports"){setDashboard(body.data);setMode(body.mode||"demo")}else setItems(body.data);
      setLoadError("");
    }catch (cause){
      if(!quiet)setLoadError(cause instanceof Error&&cause.message!=="Failed to fetch"?cause.message:"Unable to reach the hotel service. Check your connection and try again.");
  ...
  const unreadBellCount = countUniqueNotifications(dashboard?.notifications);
  useEffect(() => {
...
function ResourceView({resource,items,depositSlaHours,replenishment,createDraftPo,assets,registerAsset,assetAction,search,setSearch,open,advance,checkIn,verifyDeposit,rejectDeposit,viewReservation,viewRoom,viewGuest,processRefund,failRefund,requestApproval,escalateGuestRequest,coordinateHousekeeping,coordinateMaintenance,housekeepingAction,maintenanceAction,createReservation,openWalkIn,canMaintain,canAssignOthers,canHousekeep,canCheckIn,canVerify,canProcessRefund,canCreate,canAdvance,canCoordinate,canRequestApproval,manageRooms,onScan}:{resource:Resource;items:RecordItem[];depositSlaHours:number;replenishment:{suggestions:ReplenishmentSuggestion[];drafts:RecordItem[]}|null;createDraftPo:(s:ReplenishmentSuggestion)=>void;assets:MaintenanceAsset[]|null;registerAsset:()=>void;assetAction:(a:MaintenanceAsset,action:"record-service"|"update"|"deactivate")=>void;search:string;setSearch:(s:string)=>void;open:()=>void;advance:(x:RecordItem)=>void;checkIn:(x:RecordItem)=>void;verifyDeposit:(x:RecordItem)=>void;rejectDeposit:(x:RecordItem)=>void;viewReservation:(x:RecordItem)=>void;viewRoom:(x:RecordItem)=>void;viewGuest:(x:RecordItem)=>void;processRefund:(x:RecordItem)=>void;failRefund:(x:RecordItem)=>void;requestApproval:(x:RecordItem)=>void;escalateGuestRequest:(x:RecordItem)=>void;coordinateHousekeeping:(x:RecordItem)=>void;coordinateMaintenance:(x:RecordItem)=>void;housekeepingAction:(x:RecordItem,action:"assign"|"start"|"complete"|"inspect"|"defer"|"maintenance")=>void;maintenanceAction:(x:RecordItem,action:"assign"|"start"|"diagnose"|"defer"|"progress"|"resolve"|"close"|"cancel")=>void;createReservation:()=>void;openWalkIn:()=>void;canMaintain:boolean;canAssignOthers:boolean;canHousekeep:boolean;canCheckIn:boolean;canVerify:boolean;canProcessRefund:boolean;canCreate:boolean;canAdvance:boolean;canCoordinate:boolean;canRequestApproval:boolean;manageRooms:(()=>void)|null;onScan:(()=>void)|null}) {
  ...
  const matched=resource!=="reservations"?resource==="rooms"?items.filter((item)=>roomType==="all"||String(item.type)===roomType).sort((a,b)=>(roomStatusRank[String(a.status)]??9)-(roomStatusRank[String(b.status)]??9)||String(a.number).localeCompare(String(b.number),undefined,{numeric:true})):items:items.filter((item)=>queueFilter(item,queue,source,today));
  const ordered=["guests","inventory"].includes(resource)?sortTableRows(matched,(item)=>String(item.name??item.number??item.id)):matched;
  const page=useTablePagination(ordered);
  ...
  {resource==="reservations"&&<div className="reservation-filters"><div>{[["all","All"],["upcoming","Upcoming"],["arrivals","Arrivals today"],["departures","Departures today"],["in_house","In-house"],["cancelled","Cancelled"],["no_show","No-shows"]].map(([value,text])=><button key={value} className={queue===value?"active":""} onClick={()=>setQueue(value)}>{text}{queueCounts&&<i className="chip-count">{queueCounts.get(value)??0}</i>}</button>)}</div><div className="haven-filter"><span>Source</span><HavenSelect value={source} onChange={setSource} ariaLabel="Filter by source" options={[{ value: "all", label: "All sources" }, ...sources.map((value) => ({ value, label: value }))]} /></div></div>}
...
export function ManagerApprovalView({items,search,setSearch,review,execute,executeException,financialExecute,escalateOwner,canReview,canExecute,canFinancialExecute}:{items:RecordItem[];search:string;setSearch:(value:string)=>void;review:(item:RecordItem,decision:"approve"|"reject")=>Promise<boolean>;execute:(item:RecordItem)=>Promise<boolean>;executeException:(item:RecordItem)=>Promise<boolean>;financialExecute:(item:RecordItem)=>Promise<boolean>;escalateOwner:(item:RecordItem)=>Promise<boolean>;canReview:boolean;canExecute:boolean;canFinancialExecute:boolean}){
 ...
 const allQueue=status==="pending"?[...visible].sort(compareApprovalUrgency):visible;
 const page=useTablePagination(allQueue);
 ...
 <div className="reservation-filters approval-toolbar"><div className="approval-pills" role="group" aria-label="Filter by status">{["pending","approved","rejected","all"].map(value=><button key={value} className={status===value?"active":""} aria-pressed={status===value} onClick={()=>setStatus(value)}>{label(value)}</button>)}</div><div className="approval-filters"><label className="approval-search"><Search size={15}/><input value={search} onChange={event=>setSearch(event.target.value)} aria-label="Search approvals" placeholder="Search reservation, guest, reason, type, or requester..."/></label><div className="haven-filter"><span>Type</span><HavenSelect value={type} onChange={setType} ariaLabel="Filter by approval type" groups={[{ label: "All", options: [{ value: "all", label: "All types" }] }, ...approvalTypeGroups.map(([group, values]) => ({ label: group, options: values.filter(value=>types.includes(value)).map(value=>({ value, label: label(value) })) }))]} /></div><div className="haven-filter"><span>Department</span><HavenSelect value={department} onChange={setDepartment} ariaLabel="Filter by department" options={[{ value: "all", label: "All departments" }, ...departments.map(value=>({ value, label: label(value) }))]} /></div><div className="haven-filter"><span>Severity</span><HavenSelect value={severity} onChange={setSeverity} ariaLabel="Filter by severity" options={[{ value: "all", label: "All severities" }, { value: "normal", label: "Normal" }, { value: "high", label: "High" }, { value: "critical", label: "Critical" }]} /></div></div></div>
 ...
 <TablePagination {...page} onPageChange={page.setPage} noun="requests" allTotal={items.length} note="Urgent pending decisions stay severity-first; history stays newest-first." /></div>
...
function AccountingDetailTable({title,subtitle,ariaLabel,headers,rows,renderRow}:{title:string;subtitle:string;ariaLabel:string;headers:string[];rows:Record<string,unknown>[];renderRow:(row:Record<string,unknown>)=>React.ReactNode}){
 const page=useTablePagination(rows);
 return <article className="panel"><div className="panel-heading"><div><h3>{title}</h3><p>{subtitle}</p></div></div><div className="table-scroll"><table aria-label={ariaLabel}><thead><tr>{headers.map((header)=><th key={header}>{header}</th>)}</tr></thead><tbody>{page.rows.map(renderRow)}</tbody></table></div><TablePagination {...page} onPageChange={page.setPage} noun="records" note="Financial chronology retained."/></article>;
}
...
function AccountingView({section,ledger,search,setSearch,rejectDeposit,reverseCharge,recordAdjustment,openCashShift,closeCashShift,reconcileCashShift,recordReconciliation,generateDocument,canVerify,canAdjust,canReconcile,canOperateShift,canIssueDocument,actorId}:{section:AccountingSection;ledger:AccountingLedger|null;search:string;setSearch:(s:string)=>void;rejectDeposit:(x:RecordItem)=>void;reverseCharge:(x:RecordItem)=>void;recordAdjustment:(reservationId:string)=>void;openCashShift:()=>void;closeCashShift:(x:RecordItem)=>void;reconcileCashShift:(x:RecordItem)=>void;recordReconciliation:()=>void;generateDocument:(payload:{documentType:"receipt";paymentId:string}|{documentType:"folio";reservationId:string})=>void;canVerify:boolean;canAdjust:boolean;canReconcile:boolean;canOperateShift:boolean;canIssueDocument:boolean;actorId:string}){
  ...
  const matchingRows=ledger?(ledger[view.source] as Record<string,unknown>[]).filter((row)=>JSON.stringify(row).toLowerCase().includes(search.toLowerCase())):[];
  const page=useTablePagination(matchingRows);
  ...
  <div className="data-panel"><div className="table-scroll"><table aria-label="Ledger records"><thead><tr>{view.columns.map((column)=><th key={column.key}>{column.label}</th>)}<th>Actions</th></tr></thead><tbody>{rows.map((row)=><tr key={String(row.id)}>{view.columns.map((column)=><td key={column.key}>{column.key==="status"?<span className={`badge ${row.status}`}>{label(row.status)}</span>:column.money?<strong>{pesoExact(row[column.key])}</strong>:label(row[column.key] as string)}</td>)}<td>{actions(row)}</td></tr>)}</tbody></table></div>{matchingRows.length===0&&<div className="empty"><Search/><h3>Nothing recorded yet</h3><p>No financial records match this workspace.</p></div>}<TablePagination {...page} onPageChange={page.setPage} noun="ledger records" note={section==="transactions"?"Settlement order retained.":section==="folios"?"Every folio figure is recomputed server-side.":section==="cash_shifts"?"Expected cash is derived from recorded payments.":section==="reconciliation"?"Statement order retained.":"Documents are immutable snapshots."}/></div>
  {section==="folios"&&ledger.charges.length>0&&<AccountingDetailTable title="Recent folio charges" subtitle="Posted by hotel operations; reversible by Accounting only" ariaLabel="Recent folio charges" headers={["Reservation","Description","Category","Amount","Status","Actions"]} rows={ledger.charges} renderRow={(charge)=><tr key={String(charge.id)}><td>{label(charge.reservation_id as string)}</td><td>{label(charge.description as string)}</td><td>{label(charge.category as string)}</td><td><strong>{pesoExact(charge.amount)}</strong></td><td><span className={`badge ${charge.status}`}>{label(charge.status)}</span></td><td>{canAdjust&&charge.status!=="reversed"&&<button className="table-action" onClick={()=>reverseCharge(charge as RecordItem)}>Reverse</button>}</td></tr>}/>}
  {section==="reconciliation"&&ledger.refundAttempts.length>0&&<AccountingDetailTable title="Refund settlement attempts" subtitle="No external refund provider is connected - every attempt is operator-recorded" ariaLabel="Recent refund attempts" headers={["Attempted","Refund","Reference","Detail","Status"]} rows={ledger.refundAttempts} renderRow={(attempt)=><tr key={String(attempt.id)}><td>{label(attempt.attempted_at as string)}</td><td>{label(attempt.refund_request_id as string)}</td><td>{label(attempt.reference as string)}</td><td>{label(attempt.reason as string)}</td><td><span className={`badge ${attempt.status}`}>{label(attempt.status)}</span></td></tr>}/>}
  {section==="folios"&&ledger.adjustments.length>0&&<AccountingDetailTable title="Adjustments, credits and write-offs" subtitle="Financial corrections recorded as new entries - nothing is overwritten" ariaLabel="Recent folio adjustments" headers={["Reservation","Type","Direction","Amount","Reason","Recorded"]} rows={ledger.adjustments} renderRow={(adjustment)=><tr key={String(adjustment.id)}><td>{label(adjustment.reservation_id as string)}</td><td>{label(adjustment.transaction_type as string)}</td><td>{label(adjustment.direction as string)}</td><td><strong>{adjustment.direction==="credit"?"-":"+"}{pesoExact(adjustment.amount)}</strong></td><td>{label(adjustment.reason as string)}</td><td>{label(adjustment.created_at as string)}</td></tr>}/>}
...
function LoadFailure({message,retry}:{message:string;retry:()=>void}){return <div className="empty" role="alert"><AlertTriangle/><h3>Unable to load this module</h3><p>{message}</p><button type="button" className="btn btn-soft" onClick={retry}>Try again</button></div>}
