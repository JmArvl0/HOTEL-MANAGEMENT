...
export function ManagerApprovalView({items,search,setSearch,review,execute,executeException,financialExecute,escalateOwner,canReview,canExecute,canFinancialExecute}:{items:RecordItem[];search:string;setSearch:(value:string)=>void;review:(item:RecordItem,decision:"approve"|"reject")=>Promise<boolean>;execute:(item:RecordItem)=>Promise<boolean>;executeException:(item:RecordItem)=>Promise<boolean>;financialExecute:(item:RecordItem)=>Promise<boolean>;escalateOwner:(item:RecordItem)=>Promise<boolean>;canReview:boolean;canExecute:boolean;canFinancialExecute:boolean}){
 ...
 <div className="reservation-filters approval-toolbar"><div className="approval-pills" role="group" aria-label="Filter by status">{["pending","approved","rejected","all"].map(value=><button key={value} className={status===value?"active":""} aria-pressed={status===value} onClick={()=>setStatus(value)}>{label(value)}</button>)}</div><div className="approval-filters"><label className="approval-search"><Search size={15}/><input value={search} onChange={event=>setSearch(event.target.value)} aria-label="Search approvals" placeholder="Search reservation, guest, reason, type, or requester..."/></label><label>Type<select value={type} onChange={event=>setType(event.target.value)}><option value="all">All types</option>{approvalTypeGroups.map(([group,values])=><optgroup key={group} label={group}>{values.filter(value=>types.includes(value)).map(value=><option key={value} value={value}>{label(value)}</option>)}</optgroup>)}</select></label><label>Department<select value={department} onChange={event=>setDepartment(event.target.value)}><option value="all">All departments</option>{departments.map(value=><option key={value} value={value}>{label(value)}</option>)}</select></label><label>Severity<select value={severity} onChange={event=>setSeverity(event.target.value)}><option value="all">All severities</option><option value="normal">Normal</option><option value="high">High</option><option value="critical">Critical</option></select></label></div></div>
