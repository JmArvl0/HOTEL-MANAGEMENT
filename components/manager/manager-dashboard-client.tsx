...
import { Activity, BedDouble, Bell, Boxes, Building2, CalendarDays, CarTaxiFront, ChevronDown, CircleDollarSign, ClipboardCheck, Download, FileText, Gauge, Landmark, LogOut, Plus, ReceiptText, Scale, Search, Settings, Sparkles, Users, Wallet, Wrench, X } from "lucide-react";
...
export default function ManagerDashboardClient({ user }: { user: User }) {
  ...
  const load = useCallback(async (quiet=false) => { if(!quiet)setLoading(true); if(section==="room_types"||section==="transport_services"){if(!quiet)setLoading(false); return;} const accounting=isAccountingSection(section);const approvals=section==="approvals"; const url = section === "overview" || section === "reports" ? "/api/manager_dashboard" : accounting ? "/api/accounting/ledger" : approvals?"/api/manager/approvals":`/api/resources/${section}`; const res = await fetch(url,{cache:"no-store"}); const body = await res.json(); if (res.ok) { if (accounting) setLedger(body.data); else if (section === "overview" || section === "reports") { setDashboard(body.data); setMode(body.mode || "demo"); } else setItems(body.data); } if(!quiet)setLoading(false); }, [section]);
...
function ManagerApprovalView({items,search,setSearch,review,execute,financialExecute,escalateOwner,canReview,canExecute,canFinancialExecute}:{items:RecordItem[];search:string;setSearch:(value:string)=>void;review:(item:RecordItem,decision:"approve"|"reject")=>void;execute:(item:RecordItem)=>void;financialExecute:(item:RecordItem)=>void;escalateOwner:(item:RecordItem)=>void;canReview:boolean;canExecute:boolean;canFinancialExecute:boolean}){
