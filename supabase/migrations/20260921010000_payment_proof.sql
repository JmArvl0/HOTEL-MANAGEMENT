-- Required payment-proof image for the manual reservation deposit flow.
-- Guests must now submit BOTH a transaction reference and a proof screenshot
-- (GCash / bank transfer). Manual verification by Accounting is unchanged —
-- this only gives the verifier something to look at.
--
-- Storage: a PRIVATE `payment-proofs` bucket. No public policy, no RLS policy:
-- access happens only server-side with the service role (upload, cleanup,
-- short-lived signed URLs for the paying guest and Accounting). Proofs are
-- never exposed as permanently public URLs.

alter table public.payments add column if not exists proof_storage_path text;
alter table public.payments add column if not exists proof_original_name text;
alter table public.payments add column if not exists proof_mime_type text;
alter table public.payments add column if not exists proof_size_bytes integer;
...
-- Recreate submit_reservation_deposit with proof parameters. The body is the
-- LIVE remote body (which drifted from the 20260908 migration file: it carries
-- operational_policy_snapshot + room_is_sellable + transport folio lines),
-- extended minimally: proof columns in the payments insert and a
-- PROOF_REQUIRED guard for manual methods. Everything else is byte-identical.
drop function if exists public.submit_reservation_deposit(uuid,uuid,text,text);
