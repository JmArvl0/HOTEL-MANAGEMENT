...
const maintenanceActions=readFileSync("app/api/maintenance/orders/[id]/[action]/route.ts","utf8");
...
describe("connected workflow authorization and UI",()=>{
 it("limits refunds to Owner and Accounting",()=>{for(const role of["owner","accounting"]as const)expect(canAccess(role,"refunds")).toBe(true);for(const role of["admin","manager","front_desk","housekeeping","maintenance","guest"]as const)expect(canAccess(role,"refunds")).toBe(false)});
 it("keeps guest cancellation and modification ownership-scoped",()=>{expect(customerCancel).toMatch(/p_actor_user_id:\s*session\.user\.id/);expect(changeRequest).toMatch(/\.eq\("user_id",\s*session\.user\.id\)/);expect(changeRequest).toContain("selfServiceModificationDays")});
 it("exposes staff workflow actions without direct refund or Housekeeping-task mutation",()=>{for(const action of["Collect payment","Post charge","Complete checkout","Process refund"])expect(dashboard).toContain(action);expect(dashboard).not.toContain("Verify ID");expect(resourceRoute).toContain('["payments","refunds","housekeeping_tasks"].includes(resource)')});
});
